...
if ((int)$cot['cliente_id'] !== (int)$cliente_id) {
...
function cotEstatus(string $e): array {
    switch ($e) {
...
<title>APEX GLASS — <?= htmlspecialchars($folio) ?></title>
...
body { font-family:'Outfit',-apple-system,sans-serif; background:var(--bg); min-height:100dvh; color:var(--text-1); line-height:1.5; }
...
/* fix 3: touch target 44px + fix 2: focus-visible */
.btn-back {
  font-size:9.5px; font-weight:600; letter-spacing:2px; text-transform:uppercase;
  color:var(--text-2); background:none; border:1px solid var(--border); border-radius:2px;
  padding:0 12px; min-height:44px; cursor:pointer; text-decoration:none;
...
.btn-back:focus-visible { outline:2px solid var(--focus); outline-offset:2px; }

.btn-logout {
  font-size:9.5px; font-weight:600; letter-spacing:2px; text-transform:uppercase;
  color:var(--text-2); background:none; border:1px solid var(--border); border-radius:2px;
  padding:0 14px; min-height:44px; cursor:pointer; transition:color .15s,border-color .15s;
}
.btn-logout:hover { color:var(--text-1); border-color:#B0B5C0; }
.btn-logout:focus-visible { outline:2px solid var(--focus); outline-offset:2px; }

@media(max-width:639px) {
  .header { padding:0 18px; gap:10px; }
  .header-cliente { display:none; }
  .btn-logout { display:none; }
  .btn-back { padding:0 10px; font-size:9px; }
}

/* ── Main ── */
/* fix 1: <main> semántico */
main.wrap { max-width:1000px; margin:0 auto; padding:32px 48px 60px; }
@media(max-width:639px) { main.wrap { padding:20px 14px 48px; } }
...
/* fix 1: h1 como heading semántico */
h1.cot-folio { font-family:'Syncopate',sans-serif; font-size:15px; font-weight:700; letter-spacing:3px; color:var(--text-1); }

.tag { display:inline-flex; align-items:center; gap:6px; font-size:10px; font-weight:600; letter-spacing:1.2px; text-transform:uppercase; padding:5px 10px; border-radius:2px; white-space:nowrap; }
...
/* fix 9: ícono de estatus SVG en lugar de solo color */
...
/* fix 7: meta-label 9px→10px */
.meta-label { font-size:10px; font-weight:600; letter-spacing:2px; text-transform:uppercase; color:var(--text-3); display:block; margin-bottom:4px; }
...
/* fix 6: tabular-nums en columnas numéricas */
td.num { font-variant-numeric:tabular-nums; }

/* ── Cards mobile para partidas (fix 5) ── */
.partida-cards { display:none; flex-direction:column; gap:1px; background:var(--border); border:1px solid var(--border); border-radius:4px; overflow:hidden; margin-bottom:24px; }
.partida-card-m { background:var(--surface); padding:16px 18px; }
.pc-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px; gap:8px; }
.pc-num    { font-size:11px; font-weight:700; color:var(--text-2); letter-spacing:1px; text-transform:uppercase; }
.pc-cristal { font-size:13px; font-weight:600; color:var(--text-1); margin-bottom:6px; }
.pc-row    { display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:1px solid #F5F6F8; font-size:12px; }
.pc-row:last-of-type { border-bottom:none; }
.pc-lbl    { color:var(--text-2); }
.pc-val    { font-weight:600; font-variant-numeric:tabular-nums; }
.pc-subtotal { margin-top:10px; padding-top:10px; border-top:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; }
.pc-subtotal-lbl { font-size:10px; font-weight:600; letter-spacing:1px; text-transform:uppercase; color:var(--text-2); }
.pc-subtotal-val { font-size:15px; font-weight:700; font-variant-numeric:tabular-nums; color:var(--text-1); }

@media(max-width:639px) {
  .table-wrap { display:none; }
  .partida-cards { display:flex; }
...
.resumen-row { display:flex; justify-content:space-between; align-items:center; padding:9px 0; border-bottom:1px solid #F5F6F8; font-size:13px; }
.resumen-row:last-child { border-bottom:none; }
/* fix 8: total con más peso visual */
.resumen-row.total-row { padding-top:16px; margin-top:6px; border-top:2px solid var(--border); border-bottom:none; }
.resumen-row.total-row .r-lbl { font-size:15px; font-weight:700; color:var(--text-1); }
.resumen-row.total-row .r-val { font-size:20px; font-weight:700; color:var(--amber); font-variant-numeric:tabular-nums; }
.r-lbl { color:var(--text-2); }
.r-val { font-weight:600; color:var(--text-1); font-variant-numeric:tabular-nums; }
.r-desc { color:var(--green); font-weight:600; font-variant-numeric:tabular-nums; }

@media(max-width:639px) { .resumen-card { padding:16px 18px; } }
...
<main class="wrap">
  ...
  <div class="cot-header">
    <div class="cot-folio-row">
      <!-- fix 1: h1 para heading hierarchy -->
      <h1 class="cot-folio"><?= htmlspecialchars($folio) ?></h1>
      <!-- fix 9: SVG icon en tag de estatus -->
      <span class="tag <?= $statCls ?>" aria-label="Estatus: <?= $statTxt ?>">
        <?php if ($cot['estatus'] === 'cotizacion'): ?>
          <svg class="tag-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><circle cx="12" cy="12" r="9"/></svg>
        <?php elseif ($cot['estatus'] === 'orden'): ?>
          <svg class="tag-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        <?php elseif ($cot['estatus'] === 'entregada'): ?>
          <svg class="tag-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>
        <?php else: ?>
          <svg class="tag-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        <?php endif; ?>
        <?= $statTxt ?>
      </span>
    </div>
    ...
    <table>
      ...
      <tbody>
        <?php foreach ($partidas as $p):
          ...
          if ((int)$p['taladros_pasados'] > 0)     $trabajos[] = '<span class="ptag ptag-trab">TP&times;' . $p['taladros_pasados'] . '</span>';
          if ((int)$p['taladros_avellanados'] > 0) $trabajos[] = '<span class="ptag ptag-trab">TA&times;' . $p['taladros_avellanados'] . '</span>';
        ...
        <tr>
          ...
          <td>
            ...
            <?php if ($trabajos): ?><div style="margin-top:5px"><?= implode('', $trabajos) ?></div><?php endif; ?>
            <?php if ($p['comentarios_etiqueta']): ?>
            <div class="muted" style="margin-top:4px"><?= htmlspecialchars($p['comentarios_etiqueta']) ?></div>
            <?php endif; ?>
          </td>
          <td class="muted num"><?= $p['ancho'] ?> &times; <?= $p['alto'] ?> mm</td>
          <td class="right muted num"><?= number_format((float)$p['m2'], 4) ?></td>
          <td class="right num"><?= $p['cantidad'] ?></td>
          <td class="right muted num"><?= fmt((float)$p['precio_m2_usado']) ?></td>
          <td class="right num"><?= fmt($sub_p) ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  ...
  <!-- MOBILE: cards por partida (fix 5) -->
  <div class="partida-cards" aria-label="Partidas de la cotización">
    <?php foreach ($partidas as $p):
      $sub_p    = (float)$p['precio_m2_usado'] * (float)$p['m2'] * (int)$p['cantidad'];
      $trabajos = [];
      if ($p['cpb'])                           $trabajos[] = '<span class="ptag ptag-cpb">CPB ' . htmlspecialchars($p['cpb']) . '</span>';
      ...
      if ($p['detalles'] && $p['detalles'] !== 'NO' && $p['detalles'] !== '')
        $trabajos[] = '<span class="ptag ptag-trab">' . htmlspecialchars($p['detalles']) . '</span>';
    ?>
    <div class="partida-card-m">
      <div class="pc-header">
        <span class="pc-num">Partida <?= $p['num_partida'] ?></span>
        <?php if ($trabajos): ?><div><?= implode('', $trabajos) ?></div><?php endif; ?>
      </div>
      <div class="pc-cristal"><?= htmlspecialchars($p['cristal_nombre'] ?: '—') ?></div>
      <div class="pc-row"><span class="pc-lbl">Medidas</span><span class="pc-val"><?= $p['ancho'] ?> × <?= $p['alto'] ?> mm</span></div>
      <div class="pc-row"><span class="pc-lbl">m² por pieza</span><span class="pc-val"><?= number_format((float)$p['m2'], 4) ?></span></div>
      <div class="pc-row"><span class="pc-lbl">Cantidad</span><span class="pc-val"><?= $p['cantidad'] ?> pza</span></div>
      <div class="pc-row"><span class="pc-lbl">Precio/m²</span><span class="pc-val"><?= fmt((float)$p['precio_m2_usado']) ?></span></div>
      <?php if ($p['comentarios_etiqueta']): ?>
      <div class="pc-row"><span class="pc-lbl">Nota</span><span class="pc-val" style="font-weight:400;color:var(--text-2)"><?= htmlspecialchars($p['comentarios_etiqueta']) ?></span></div>
      <?php endif; ?>
      <div class="pc-subtotal">
        <span class="pc-subtotal-lbl">Subtotal partida</span>
        <span class="pc-subtotal-val"><?= fmt($sub_p) ?></span>
      </div>
    </div>
    <?php endforeach; ?>
  </div>
