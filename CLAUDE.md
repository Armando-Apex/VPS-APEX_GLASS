...
# Última actualización: 16 septiembre 2026 | Próximo UPD disponible: UPD-590
...
| MEDIA | Armando | **Descuento por Encuesta de Satisfacción (UPD-588/589) — verificado de punta a punta con una respuesta real, falta solo prueba visual en navegador.** Flow publicado en Meta (16-sep-2026), plantilla nueva `encuesta_satisfaccion_v2` aprobada (categoría MARKETING, a propósito — Armando confirmó que solo se manda una vez a los 350 clientes del CRM, no hay riesgo de tope de frecuencia por repetición al mismo cliente). Prueba real ejecutada: mensaje enviado al número de prueba de Armando, respuesta capturada, código `ENC-B12B5E` generado y entregado por WhatsApp — confirmado en BD. Reporte de concentrado construido en Reporte Dirección → Comercial (UPD-589). Falta: (1) prueba visual — ver el campo "Código de Encuesta" en Cotización y la tabla del reporte en el navegador; (2) decidir si se sigue usando la plantilla vieja `encuesta_clientes` (Flow de 3 preguntas) en paralelo o se retira; (3) armar y mandar la campaña real a los 350 clientes cuando Armando lo autorice | Pendiente — falta prueba visual + autorización para el envío a los 350 |
...
Próximo UPD disponible: **UPD-590**
