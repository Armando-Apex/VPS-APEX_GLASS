...
# Última actualización: 13 julio 2026 | Próximo UPD disponible: UPD-335
...
| ALTA | Mando | **GPS ProTrack365 en Logística Rutas (ver UPD-327/328)** — `api/gps_proxy.php` ya no está bloqueado: se agregó fallback automático (login web + token, sin captura manual) que funciona mientras la Open API oficial siga en `permission denied`; mapeo gris/blanca ya resuelto por `imei`/`devicename` reales. Falta: (1) decidir consulta en vivo vs. histórico en tabla `gps_posiciones` + cron, (2) conectar al frontend de `logistica_rutas.php` (aún sin cambios), (3) seguir pidiendo al distribuidor la Open API oficial para no depender del endpoint no documentado a largo plazo | Parcial — backend listo, falta frontend |
...
**Próximo UPD disponible: UPD-330**
