...
# Última actualización: 07 agosto 2026 | Próximo UPD disponible: UPD-474
...
| UPD-472 | 07-ago-2026 | Armando | Fix "Rentabilidad por m² de vidrio" (Reporte Dirección) — Armando preguntó si el precio real de venta tomaba el precio de julio; se confirmó que usaba una **ventana fija desde 01-jul-2026 hasta hoy** (`api/inventario.php`, `$FECHA_INICIO_PRECIO_REAL`, UPD-314), a diferencia del costo de compra de láminas que ya usa mes calendario en curso (rolling, UPD-389). Armando pidió que el precio de venta también sea por mes en curso, ya que "el análisis es por mes" — con la ventana fija, el incremento de precios del 3-ago (UPD-448) se estaba mezclando con ventas de julio (más baratas) en un solo promedio, mostrando un "precio real" por debajo del catálogo vigente. Fix: `$FECHA_INICIO_PRECIO_REAL` ahora reutiliza `$MES_ACTUAL_INICIO` (rolling, ya calculado arriba en el mismo archivo para el costo) — ambos lados del cuadro (costo y precio de venta) usan la misma ventana de mes en curso. El subtítulo del cuadro en `app/modulos/reporte_direccion.php` ya mostraba la fecha dinámicamente (`precio_real_desde` del API), no requirió cambio. Verificado contra BD real: Claro 9mm en agosto (01-ago a la fecha) da $861.46/m² real — ya refleja el incremento de precios, en vez de seguir arrastrado por julio. |

| UPD-473 | 07-ago-2026 | Armando | Fix de seguimiento a UPD-472: Armando probó cambiar el selector de período del Reporte Dirección a "Mes anterior" y el cuadro de Rentabilidad seguía mostrando los datos de agosto — causa real: `api/inventario.php?accion=costo_promedio` (de donde sale TODO el cuadro, costo y precio real) nunca recibía el `periodo` seleccionado; siempre calculaba "mes calendario de hoy" a fuerza, sin importar qué eligiera el usuario en el reporte. Fix: `app/modulos/reporte_direccion.php` ahora manda `&periodo=` en ese fetch (mismo valor que ya usa para `api/reporte_direccion.php`); `api/inventario.php` resuelve ese `periodo` con la misma lógica de rangos que `api/reporte_direccion.php` (mes_actual/mes_anterior/3meses/6meses/año/todo) y la usa tanto para el costo de compra de láminas como para el precio real de venta (antes de UPD-472 este último ya tenía ventana propia fija desde jul-01, ahora ambos comparten el rango del selector). Verificado contra BD real: Claro 9mm dio $754.41/m² con "Mes anterior" (julio) vs $861.46/m² con "Este mes" (agosto) — el cuadro ya responde al selector. |

**Próximo UPD disponible: UPD-474**
