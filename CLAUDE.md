...
| ALTA | Mando | **GPS ProTrack365 en Logística Rutas (ver UPD-327)** — bloqueante real: `api/gps_proxy.php` (ya construido, flujo oficial correcto) responde `permission denied` porque la cuenta `vitrotempladomty` no tiene la Open API habilitada del lado de ProTrack365. Pedir al distribuidor que la habilite; en cuanto responda `code:0`, el endpoint ya funciona sin tocar código. Después: (1) decidir consulta en vivo vs. histórico en tabla `gps_posiciones` + cron, (2) conectar al frontend de `logistica_rutas.php`, (3) cruzar `deviceid` (visto en método alterno, 1525959/1550184) con IMEI gris/blanca de `.env` para saber cuál unidad es cuál | Pendiente (13-jul-2026) |
...
**Próximo UPD disponible: UPD-328**
