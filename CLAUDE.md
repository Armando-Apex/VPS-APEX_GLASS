...
# Última actualización: 13 julio 2026 | Próximo UPD disponible: UPD-324
...
| ALTA | Armando | QR de salida por chofer (UPD-319) — código listo, faltan 2 cosas para que mande WhatsApp real: (1) crear en Meta Business Manager las 4 plantillas `chofer_en_ruta_cliente`/`siguiente_entrega_cliente`/`chofer_en_ruta_asesor`/`siguiente_entrega_asesor` (categoría UTILITY, textos sugeridos en UPD-319); (2) dar los números reales de WhatsApp de Bethy y Cynthia para cargarlos en `usuarios.telefono` (columna ya existe, vacía). También pendiente: renombrar `usuarios.nombre` de chofer1/chofer2 a los nombres reales si se quiere que el mensaje diga el nombre del chofer, y una prueba visual con un chofer real/QR físico (no se pudo probar por HTTP esta sesión — la técnica de sesión simulada usada en sesiones previas fue bloqueada por el clasificador de seguridad; se verificó la lógica completa a nivel de BD con datos de prueba en su lugar) | Pendiente |
...
**Próximo UPD disponible: UPD-325**
